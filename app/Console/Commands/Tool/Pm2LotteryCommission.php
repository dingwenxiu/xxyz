...
class Pm2LotteryCommission extends Command {

    protected $signature = 'tool:pm2LotteryCommission {action} {--redis=}';
    protected $description = 'gen 生成配置文件,stop:停止,start:启动, restart:重启';

	public function handle()
	{
        $path       = __DIR__ . '/../Runtime/';
        $pm2_tpl    = __DIR__ . '/tpl/pm2.lottery.commission.tpl.php';

        $gen_pm2_path = $path . 'pm2.lottery.commission.json';
	...
	public function check() {
        ...
        if (count($process) > 0) {
            foreach ($process as $item) {
                $this->info("已存在 lottery commission 进程-" . $item);
	...
	public function start($gen_pm2_path) {
        $this->info("开启 lottery commission 脚本 ......");
    ...
    public function stop($gen_pm2_path) {
        $this->info("结束 lottery commission 脚本 ......");
    ...
    public function gen($pm2Tpl, $path, $gen_pm2_path) {
        $this->info("生成 lottery commission 的配置文件!");

        $commissionConfigArr    = config("command.commission");
