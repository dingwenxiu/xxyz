 ...
 */
class CronCommission extends Command {

    protected $signature    = 'tool:cron_commission {action}';
    ...
    public function __construct() {
        parent::__construct();
        $this->tpl    = __DIR__ . '/tpl/cron_commission.tpl.php';
        $this->path   = __DIR__ . '/../Runtime/';
        $this->file   = $this->path . 'cron_commission.conf';

        $md5 = md5(config('app.name'));
        $this->startMark =   'start_mark_commission_' . $md5;
        $this->endMark   =   'end_mark_commission_' . $md5;
    }
    ...
    public function gen() {
        ...
        $commands = config("command.commission");
        if(empty($commands)){
