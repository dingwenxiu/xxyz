...
class Pm2LotteryOpen extends Command {

    protected $signature = 'tool:pm2LotteryOpen {action} {--redis=}';
    protected $description = 'gen 生成配置文件,stop:停止,start:启动, restart:重启';

	public function handle()
	{
        $path       = __DIR__ . '/../Runtime/';
        $pm2_tpl    = __DIR__ . '/tpl/pm2.lottery.open.tpl.php';

        $gen_pm2_path = $path . 'pm2.lottery.open.json';
	...
	public function check() {
        ...
        if (count($process) > 0) {
            foreach ($process as $item) {
                $this->info("已存在 lottery open 进程-" . $item);
    ...
    public function gen($pm2Tpl, $path, $gen_pm2_path) {
        $this->info("生成 lottery Open 的配置文件!");

        $openConfigArr    = config("command.open");
