...
class LotteryCommission extends BaseGame
{
    protected $table = 'lottery_commissions';
    ...
    static function getList($c, $pageSize = 15) {
        ...
        $query = self::whereBetween('add_time',[$timeNow,$timeFuture])->orderBy('id', 'desc');
     ...
     * @return array
     */
    static function getIssueCommissionCount($lotterySign, $issueNo) {
        $set = self::where('lottery_sign', $lotterySign)->where('issue', $issueNo)->whereIn('status', [0, 1])->get();

        $data = [
            'total_bet_commission'      => 0,
            'total_child_commission'    => 0,
        ];

        foreach ($set as $item) {
            if ($item->from_type == 'self') {
                $data['total_bet_commission'] += $item->amount;
            } else {
                $data['total_child_commission'] += $item->amount;
            }
        }
    ...
    static function getPlayerDaySum($playerId, $day) {
        ...
        $items = self::where('user_id', $playerId)->whereBetween('add_time', [$startTime, $endTime])->get();
        $data = [
